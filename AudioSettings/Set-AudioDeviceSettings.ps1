...
.DESCRIPTION
    ...
    PERMISSIONS
       MMDevices keys are owned by NT SERVICE\AudioEndpointBuilder. Even SYSTEM cannot write
       them without privilege escalation. This script enables SeTakeOwnershipPrivilege and
       SeRestorePrivilege via P/Invoke, takes ownership of the Render and Capture parent keys
       with full inheritable permissions for SYSTEM, then performs the writes. Ownership is
       taken on the parent key with ContainerInherit+ObjectInherit so all device subkeys are
